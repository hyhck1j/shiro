  ...
  <div class="admin-content">
    <div class="admin-content-body">
      ...
      <ul class="am-avg-sm-1 am-avg-md-4 am-margin am-padding am-text-center admin-content-list ">
        <li><a href="../../admin/resource/add.do" class="am-text-success">新增资源</a></li>
        <li><a href="javascript:void(0);" class="am-text-warning del">删除资源</a></li>
...
{{if rows}}
	{{each rows as row i}}
		<tr>
			<td><input type="checkbox" class="item" data-id="{{row.resourceId}}"/></td>
			<td>{{row.resourceName}}</td>
			<td>{{row.resourceUrl}}</td>
			<td><a href="../../admin/resource/edit/{{row.resourceId}}.do" class="am-btn am-btn-primary resource-edit" data-id="{{row.resourceId}}">编辑</a></td>
		</tr>
	{{/each}}
{{/if}}
</script>
...
$(function(){
	...
	$('#pagination-page').pagination({
	    ...
	    dataSource: '../../admin/resource/pagination.do',
	    locator: 'data',
	...
	$('.del').off('click').on('click',function(){
		...
		if(confirm("数据删除后无法恢复，您确认要删除吗?")){
			...
			$.post('/admin/resource/del.do',{resourceIds:ids.join(',')},function(res){
				console.debug(res) ;
				if(res.code == 'error'){
					alert(res.message) ;
				}else{
